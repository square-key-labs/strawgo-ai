...
- **`ServiceSwitcher` failover composite** — new `src/services/switcher/failover.go` wrapping multiple `AIService` instances. Routes incoming frames to the active service, intercepts upstream `ErrorFrame` produced by the inner service, and on a non-fatal error: cleans up the failed service, initializes the next one, and propagates the original `ErrorFrame` upstream as informational. Fatal errors (`ErrorFrame.IsFatal() == true`) are propagated unchanged with no switch. Does NOT replay frames — downstream consumers may have observed partial output of the failed service. `WithOnSwitch` registers a callback for service-switched events. Mirrors pipecat `service_switcher_strategy_failover`. (`src/services/switcher/failover.go`)
- **`ErrorFrame.IsFatal()` accessor** — reads the `MetadataKeyFatal` boolean populated by `BaseProcessor.PushError(..., fatal=true)`. Returns false when metadata is absent or non-bool. Used by `ServiceSwitcher` to distinguish recoverable failures from pipeline-fatal ones. (`src/frames/system.go`, `src/processors/processor.go`)
- **`LLMUpdateSettingsFrame` / `TTSUpdateSettingsFrame`** — runtime settings update frames carrying a free-form `Settings map[string]interface{}` plus an optional `Service` target field that scopes the update to a single service instance (matches the `STTUpdateSettingsFrame` shape introduced in PR 3). Carried as `SystemFrame` so they jump ahead of queued data/control frames. Wired into OpenAI LLM, Anthropic LLM, Cartesia TTS, and ElevenLabs TTS via new `UpdateSettings(map[string]interface{}) error` methods. Mirrors pipecat #4004. Other LLM/TTS services can adopt the same `HandleFrame` pattern. (`src/frames/control.go`, `src/services/openai/llm.go`, `src/services/anthropic/llm.go`, `src/services/cartesia/tts.go`, `src/services/elevenlabs/tts.go`)
- **`WebSocketConfig.AudioOutAutoSilence`** — optional `*bool` (default `true`) governing the WebSocket transport's VAD silence-timeout heuristic. When false, the chunk sender does not reset the VAD timer after each audio chunk, so the gap-based "bot stopped speaking" inference never fires; bot-stopped detection then depends entirely on explicit `PlaybackCompleteFrame` signals (or the existing 10s fallback timer). Use this when the synthesis upstream pads its own silence and the heuristic would mis-fire mid-utterance. Mirrors pipecat `audio_out_auto_silence`. (`src/transports/websocket.go`)

- **Concurrent TTS audio contexts** — opt-in mode that pipelines per-sentence synthesis under independent `context_id`s. Three coordinated changes: (1) `AudioContextManager` gains `ConcurrentContexts` flag plus a set-based `activeContexts map[string]struct{}` and methods `NewContextID`, `RegisterContext`, `RemoveContext`, `RemoveContextAndIsEmpty` (atomic remove+empty check), `IsActiveContext`, `ActiveContextIDs`, plus `NewConcurrentAudioContextManager()` constructor. Existing single-context callers preserved. (2) `CartesiaTTS.TTSConfig.ConcurrentContexts` and `ElevenLabsTTS.TTSConfig.ConcurrentContexts`: when enabled, every `synthesizeText` invocation registers a fresh context, emits its own `TTSStartedFrame`, and on `LLMFullResponseEndFrame` flushes/closes every active context independently. Per-context "done"/"isFinal" arrivals push a downstream `TTSStoppedFrameWithContext(id)` and atomically prune via `RemoveContextAndIsEmpty` so the per-turn `isSpeaking` flag only flips once the last context drains. (3) `WebSocketConfig.ConcurrentTTSContexts`: switches the output processor's audio-filter from a single-slot model (`expectedContextID` + `currentContextID`) to a queue/set model (`expectedContexts` + `currentContexts` + `audioBuffersByCtx`). Audio frames are admitted if their `context_id` is in the expected or current set; first-time admits promote into current. Per-context audio buffers prevent sub-chunk-size remainders from one sentence bleeding bytes into another. `TTSStoppedFrameWithContext(id)` prunes the ID from every per-context structure; `InterruptionFrame` wipes them all. Mirrors pipecat's v0.0.105 concurrent-audio-context mode. ElevenLabs additionally gains a `wsMu` write mutex (gorilla allows one concurrent writer; concurrent mode adds many writer call-sites — synthesis, keepalive, interruption, flush, close) plus a 10s per-write deadline so a hung peer cannot wedge `Cleanup`. Default off; legacy single-context behavior unchanged. (`src/services/service.go`, `src/services/cartesia/tts.go`, `src/services/elevenlabs/tts.go`, `src/transports/websocket.go`, `src/frames/control.go`)
...
- **`group_parallel_tools` batching** — when an LLM response triggers multiple tool calls in a single batch, the assistant aggregator now triggers exactly one follow-up LLM inference after every call in the group has reported a result, instead of triggering once per result. The `FunctionCallsStartedFrame` carries a `GroupID` (auto-generated by the aggregator if the producer leaves it empty) which propagates onto the resulting `FunctionCallInProgressFrame` and `FunctionCallResultFrame` instances. Mirrors pipecat #4217. Backwards compatible — legacy producers that emit no `FunctionCallsStartedFrame` still trigger the LLM under the original empty-pending gate. (`src/frames/control.go`, `src/processors/aggregators/assistant.go`)
- **Async function calls (`CancelOnInterruption=false`)** — when a `FunctionRegistry` registration marks a tool as async, the aggregator: (1) lets the LLM continue immediately without waiting for the result, and (2) injects the late-arriving result into the conversation context as a `developer`-role message and triggers a new LLM inference at that point. This unblocks long-running tools (search, document fetch, web actions) that would otherwise stall the conversation. Mirrors pipecat #4217 / #4230. (`src/processors/aggregators/assistant.go`)
- **Per-tool `TimeoutSecs`** — `RegisteredFunction.TimeoutSecs` (per-tool) and `AssistantAggregatorParams.DefaultFunctionCallTimeout` (service-level fallback) wire into the assistant aggregator. Calls that exceed their timeout cause a `FunctionCallCancelFrame` to be pushed upstream so the LLM service can release its in-flight handle. Matches pipecat `function_call_timeout_secs` semantics, including the v1.0.0 default of "no timeout unless explicitly configured". (`src/processors/aggregators/assistant.go`)
- **`AssistantAggregatorParams.FunctionRegistry` / `DefaultFunctionCallTimeout`** — optional fields. When the registry is provided, the aggregator overrides each in-progress frame's `CancelOnInterruption` flag from the registry, so LLM services don't need to know which tools are async. Both fields are nil-safe; existing callers that build `AssistantAggregatorParams` without them keep working unchanged. (`src/processors/aggregators/assistant.go`)
- **`STTUpdateSettingsFrame`** — runtime STT settings update frame with optional `Service` target field. Wired into Deepgram, Azure, and AssemblyAI STT services via a new `UpdateSettings(map[string]interface{}) error` method. Recognized keys are provider-specific: Deepgram (`language`, `model`, `encoding`), Azure (`language`), AssemblyAI (`language`, `model`, `domain`). New values take effect on the next reconnect for providers that don't support mid-stream updates. (`src/frames/control.go`, `src/services/deepgram/stt.go`, `src/services/azure/stt.go`, `src/services/assemblyai/stt.go`)
- **`UserTurnStopStrategyV2` interface** — non-breaking richer interface that returns a `StopResult` enum (`Continue`, `Stop`, `StopShortCircuit`). The aggregator iteration prefers V2 when implemented, falling back to the bool `ShouldStop` for legacy strategies. Mirrors pipecat #4064. Existing strategies keep working unchanged. (`src/turns/user_stop/strategy.go`, `src/processors/aggregators/user.go`)
- **VAD `stop_secs` vs STT TTFS-P99 warning** — `SpeechTimeoutUserTurnStopStrategy.SetVADStopSecs()` lets the strategy know what the VAD analyzer is configured for. When the observed STT TTFS-P99 latency reaches the strategy and `vad_stop_secs >= p99`, a warning is emitted exactly once per (vad_stop_secs, p99) pair. Mirrors pipecat #4115. Includes a `SetWarnFunc` test hook. (`src/turns/user_stop/speech_timeout.go`)
