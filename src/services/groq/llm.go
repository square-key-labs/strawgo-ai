...
// GroqLLMService provides language model capabilities using Groq's OpenAI-compatible API
...
const (
	// DefaultGroqBaseURL is the default Groq API endpoint (OpenAI-compatible)
	DefaultGroqBaseURL = "https://api.groq.com/openai/v1"
	// DefaultGroqModel is the default Groq model
	DefaultGroqModel = "llama-3.3-70b-versatile"
)

// NewGroqLLMService creates a new Groq LLM service
func NewGroqLLMService(config GroqLLMConfig) *GroqLLMService {
...
func (s *GroqLLMService) SetModel(model string) {
	s.model = model
}

func (s *GroqLLMService) SetSystemPrompt(prompt string) {
	s.context.SystemPrompt = prompt
}

func (s *GroqLLMService) SetTemperature(temp float64) {
	s.temperature = temp
}

func (s *GroqLLMService) AddMessage(role, content string) {
...
func (s *GroqLLMService) ClearContext() {
	s.context.Clear()
}

func (s *GroqLLMService) Initialize(ctx context.Context) error {
	s.ctx, s.cancel = context.WithCancel(ctx)
	log.Printf("[Groq] Initialized with model %s", s.model)
	return nil
}
...
func (s *GroqLLMService) HandleFrame(ctx context.Context, frame frames.Frame, direction frames.FrameDirection) error {
	...
	if _, ok := frame.(*frames.InterruptionFrame); ok {
		...
		log.Printf("[Groq] 🔴 INTERRUPTION received (isGenerating=%v, timeSinceContext=%v)", s.isGenerating, timeSinceContext)

		if isNewContext {
			// This interruption is for the OLD response, not the new one we just started
			log.Printf("[Groq] ⚪ Ignoring interruption - new context was just received (%v ago)", timeSinceContext)
			s.streamMu.Unlock()
			return s.PushFrame(frame, direction)
		}

		if s.isGenerating && s.requestCancel != nil {
			log.Printf("[Groq] 🔴 CANCELLING ongoing stream")
			s.requestCancel()
	...
	if contextFrame, ok := frame.(*frames.LLMContextFrame); ok {
		...
		if llmContext, ok := contextFrame.Context.(*services.LLMContext); ok {
			log.Printf("[Groq] Received LLMContextFrame with %d messages", len(llmContext.Messages))
			...
			if err := s.generateResponseFromContext(llmContext); err != nil {
				...
				if s.requestCtx != nil && s.requestCtx.Err() == context.Canceled {
					log.Printf("[Groq] Stream cancelled by interruption")
				} else {
					log.Printf("[Groq] Error generating response: %v", err)
...
func (s *GroqLLMService) generateResponseFromContext(llmCtx *services.LLMContext) error {
	...
	for scanner.Scan() {
		...
		case <-s.requestCtx.Done():
			log.Printf("[Groq] Stream interrupted, stopping generation")
			return nil
		default:
		}
	...
	if response != "" {
		...
		log.Printf("[Groq] Assistant: %s", response)
	}
