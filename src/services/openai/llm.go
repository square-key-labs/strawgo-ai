...
// UpdateSettings applies a runtime settings update to the OpenAI LLM service.
// Recognized keys: "model" (string), "temperature" (float64 or int),
// "system_instruction" (string). Unknown keys are ignored with a debug log.
...
func (s *LLMService) HandleFrame(ctx context.Context, frame frames.Frame, direction frames.FrameDirection) error {
	// Honor LLMUpdateSettingsFrame. Targeted (Service set) frames apply only
	// when the target matches our processor name; untargeted frames apply to
	// every LLM service downstream of the producer. Forward the frame so
	// other LLM services in the pipeline (e.g. behind a ServiceSwitcher)
	// can also see it.
