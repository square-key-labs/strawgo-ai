...
type VADParams struct {
	...
	// transitioning from SPEAKING to QUIET (default: 0.2)
	StopSecs float32
...
func DefaultVADParams() VADParams {
	return VADParams{
		...
		StartSecs:  0.2,
		StopSecs:   0.2,
		MinVolume:  0.1, // Lower default to catch most speech, only filter complete silence
	}
}
...
func (v *BaseVADAnalyzer) ProcessAudio(buffer []byte, voiceConfidence float32, numFramesRequired int) (VADState, error) {
	...
	if v.smoothedVolume < v.params.MinVolume {
		// Log when voice is filtered due to low volume (helps diagnose VAD issues)
		if voiceConfidence >= v.params.Confidence {
			logger.Debug("[VADAnalyzer] Voice filtered: volume=%.3f < minVolume=%.3f (confidence was %.3f)",
				v.smoothedVolume, v.params.MinVolume, voiceConfidence)
		}
		voiceConfidence = 0.0
	}
	...
	case VADStateStarting:
		if voiceConfidence >= v.params.Confidence {
			v.startFrames++
			v.stopFrames = 0 // Reset stop counter on voice detected
			if v.startFrames >= v.startThreshold {
				v.state = VADStateSpeaking
				v.startFrames = 0
			...
			v.stopFrames++
			startingStopThreshold := v.stopThreshold / 4
			if startingStopThreshold < 2 {
				startingStopThreshold = 2 // Minimum 2 frames (~64ms at 8kHz)
			}
			if v.stopFrames >= startingStopThreshold {
				v.state = VADStateQuiet
				v.startFrames = 0
				v.stopFrames = 0
				logger.Debug("[VADAnalyzer] STARTING → QUIET (confidence=%.3f, volume=%.3f, stopFrames=%d)",
					voiceConfidence, v.smoothedVolume, v.stopFrames)
			}
		}
