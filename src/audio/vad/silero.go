...
// initONNXRuntimeForVAD initializes ONNX Runtime once for the VAD package
func initONNXRuntimeForVAD() error {
	ortInitOnce.Do(func() {
		// Set ONNX Runtime library path
		// Try project lib directory first, then system paths
		libPath := "lib/libonnxruntime.1.20.1.dylib"
		...
		// Ignore "already initialized" error - another component may have initialized it
		if ortInitErr != nil && strings.Contains(ortInitErr.Error(), "already been initialized") {
			logger.Debug("[SileroVAD] ONNX Runtime already initialized by another component")
			ortInitErr = nil
		}
	})
	return ortInitErr
...
// Downloads the model from HuggingFace on first use and caches locally
func NewSileroOnnxModel() (*SileroOnnxModel, error) {
	// Initialize ONNX Runtime (only once across all instances)
	if err := initONNXRuntimeForVAD(); err != nil {
		return nil, fmt.Errorf("failed to initialize ONNX environment: %w\n\nTo install ONNX Runtime:\n  macOS ARM64: curl -L https://github.com/microsoft/onnxruntime/releases/download/v1.20.1/onnxruntime-osx-arm64-1.20.1.tgz -o /tmp/onnx.tgz && tar -xzf /tmp/onnx.tgz -C /tmp && sudo cp /tmp/onnxruntime-osx-arm64-1.20.1/lib/libonnxruntime.*.dylib /usr/local/lib/\n  macOS x86_64: curl -L https://github.com/microsoft/onnxruntime/releases/download/v1.20.1/onnxruntime-osx-x86_64-1.20.1.tgz -o /tmp/onnx.tgz && tar -xzf /tmp/onnx.tgz -C /tmp && sudo cp /tmp/onnxruntime-osx-x86_64-1.20.1/lib/libonnxruntime.*.dylib /usr/local/lib/\n  Linux: sudo apt-get install libonnxruntime-dev", err)
	}

	// Download model on first use (cached for subsequent calls)
	modelPath, err := models.EnsureModel(models.SileroVADURL, models.SileroVADFile)
	if err != nil {
		return nil, fmt.Errorf("failed to get Silero VAD model: %w", err)
	}
...
func NewSileroVADAnalyzer(sampleRate int, params VADParams) (*SileroVADAnalyzer, error) {
	...
	analyzer := &SileroVADAnalyzer{
		...
		model:           model,
		logEveryNFrames: 50, // Log VAD stats every 50 frames (~1.6s at 8kHz)
	}
...
func (v *SileroVADAnalyzer) AnalyzeAudio(buffer []byte) (VADState, error) {
	...
	// Calculate volume for debugging (same calculation as BaseVADAnalyzer)
	volume := calculateVolumeFromFloat32(v.bytesToFloat32(buffer))
	...
	// Periodic debug logging to show what VAD is seeing
	v.frameCount++
	if v.logEveryNFrames > 0 && v.frameCount%v.logEveryNFrames == 0 {
		params := v.GetParams()
		logger.Debug("[SileroVAD] Stats: confidence=%.3f (thresh=%.2f), volume=%.3f (thresh=%.2f), state=%s",
			confidence, params.Confidence, volume, params.MinVolume, state.String())
	}

	return state, nil
}

// bytesToFloat32 converts int16 buffer to float32 normalized to [-1.0, 1.0]
func (v *SileroVADAnalyzer) bytesToFloat32(buffer []byte) []float32 {
